...
def determine_fuzzy_set(value, intervals):
    # TMF simetris untuk interval tengah, trapesium ujung disederhanakan
    memberships = []
    for i,(lo,hi) in enumerate(intervals):
        if i==0:
            a, b, c = lo, hi, hi+(hi-lo) # puncak di hi
        elif i==len(intervals)-1:
            a, b, c = lo-(hi-lo), lo, hi # puncak di lo
        else:
            a, b, c = lo, (lo+hi)/2, hi
...
# PSO util (hanya dipakai jika aktif)
def _fitness_boundaries(inner_bounds, data, u_min, u_max, k):
    # inner_bounds shape: (n_particles, k-1)
    costs = []
    for row in inner_bounds:
        ...
        if len(fuzz) < 2:
            costs.append(np.inf)
            continue
        # naive one-step ahead: gunakan state sebelumnya
        y_true = fuzz[1:]
        y_pred = fuzz[:-1]
        # pakai indeks set sebagai angka agar bisa RMSE (approx)
...
if run_pso:
    ...
    # tanpa PSO: bagi rata (equal-width) di rentang UoD
...
for obat, sub in df.groupby("OBAT"):
    ...
    predicted_fs = flrg.get(last_fs, [last_fs])  # fallback: pakai state terakhir
...
if uploaded_real is not None:
    ...
    # Normalisasi nama: harapkan OBAT & REALISASI_AKTUAL
    # Jika beda, silakan ubah di sini:
    if "OBAT" not in df_real.columns:
        ...
        # Ambil prediksi utk bulan prediksi
